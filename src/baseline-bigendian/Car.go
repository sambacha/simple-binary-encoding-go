...
func (c *Car) RangeCheck(actingVersion uint16, schemaVersion uint16) error {
	...
	if !utf8.Valid(c.ActivationCode[:]) {
		return errors.New("c.ActivationCode failed UTF-8 validation")
	}
	return nil
}
...
func (c *CarFuelFigures) RangeCheck(actingVersion uint16, schemaVersion uint16) error {
	...
	if !utf8.Valid(c.UsageDescription[:]) {
		return errors.New("c.UsageDescription failed UTF-8 validation")
	}
	return nil
}
